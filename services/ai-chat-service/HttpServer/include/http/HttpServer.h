...
public:
    ...
    using StreamCallback = std::function<void (const muduo::net::TcpConnectionPtr&, const http::HttpRequest&)>;
    ...
    void PostStream(const std::string& path, const StreamCallback& cb);
...
private:
    ...
    bool handleStreamRequest(const muduo::net::TcpConnectionPtr& conn, const HttpRequest& req);
    static std::string routeKey(HttpRequest::Method method, const std::string& path);
...
private:
    ...
    std::unordered_map<std::string, StreamCallback> streamCallbacks_;
